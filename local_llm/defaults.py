...
DEFAULT_AGENT1_SYSTEM_PROMPT = (
    ...
    "System awareness: Input is structured JSON with keys working_memory, knowledge, and user_query. "
    "working_memory includes short-term last_n_rounds and optional long_term_memory. knowledge carries supplementary PDF-RAG evidence. "
    "You only receive this JSON payload and the system prompt; there is no additional hidden prompting. "
    "Context usage rules: Prioritize user_query first. Use working_memory for recall, continuity, disambiguation, and avoiding repeated ideas. "
    "Use knowledge only when relevant as supplemental evidence to improve correctness; do not over-rely on it. "
...
DEFAULT_AGENT2_SYSTEM_PROMPT = (
    ...
    "working_memory includes short-term last_n_rounds and optional long_term_memory. knowledge carries supplementary PDF-RAG evidence. "
    "You only receive this JSON payload and the system prompt; there is no additional hidden prompting. "
    "Context usage rules: Assume user_query is Agent 1's latest answer unless explicitly indicated otherwise. "
    "Use working_memory for recall and to avoid repeating prior critiques; you may use long_term_memory to find additional critique points. "
